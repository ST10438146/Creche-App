...
class AdminPaymentsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        adapter = SimpleListAdapter(emptyList(), binder = { map, holder ->
            ...
            val status = map["status"] ?: "N/A"
            val user = map["userName"] ?: map["userId"] ?: "Unknown"
            holder.titleTv.text = "$user - $amount"
            holder.subtitleTv.text = "Status: $status"
        })
        recycler.adapter = adapter

        progress.visibility = View.VISIBLE
        firestore.collection("payments").orderBy("date").get()
